...
def em(sln0: Selection, data: Dataset, alpha=1.0, em_iterations=5, seed=42, N_E=1e4):
    def trans(logit_p1, p0, s):
        ...
        # note: bce = loss = -ll
        ret = 2 * N_E * -bce(label=p0, logit=logit_pp)
        ret = jnp.where(~jnp.isfinite(logit_p1), 0.0, ret)
        return ret.sum()
    ...
    def obj(sln, args):
        ...
        lls = f(logit_paths[:, :-1], jax.nn.sigmoid(logit_paths[:, 1:]))
        BOUND = 0.1
        bound_pen = jax.nn.relu(jnp.abs(s) - BOUND).sum()
        ret = -lls.mean() + 100.0 * bound_pen + jnp.sum(s * s)
