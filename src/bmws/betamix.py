...
from jax.scipy.special import betaln, digamma, gammaln, logit, logsumexp, xlog1py, xlogy
...
def scan(f, init, seq, reverse=False):
    ...
    accum = init
    ret = []
    batch_size = jax.tree.reduce(
        lambda x, y: x, jax.tree.map(lambda x: x.shape[0], seq)
    )
...
def random_binomial_large_N(key, N, p, no_boundary=True):
    ...
    ret = jnp.select([N * p < 10.0, N * (1 - p) < 10.0], [y_pois0, N - y_pois1], y_norm)
...
class EarlyStop(optx.AbstractMinimiser):
    ...
    def init(self, *args, **kwargs):
        ...
        return dict(
            ...
            patience=jnp.array(self.patience),
        )

    def step(self, fn, y, args, options, state, tags):
        y1, state["base_state"], aux = self.solver.step(
            fn, y, args, options, state["base_state"], tags
        )
    ...
    def terminate(self, fn, y, args, options, state, tags):
        stop, result = self.solver.terminate(
            fn, y, args, options, state["base_state"], tags
        )
    ...
    def postprocess(self, fn, y, aux, args, options, state, tags, result):
        return self.solver.postprocess(
            fn, y, aux, args, options, state["base_state"], tags, result
        )
...
class KDEDistribution(NamedTuple):
    ...
    def _bw(kde):
        ...
        def f(h):
            kde_train = jax.scipy.stats.gaussian_kde(
                s_train.T, weights=w_train, bw_method=h
            )
    ...
    def _kde(self):
        return jax.scipy.stats.gaussian_kde(
            self.data.T, weights=self.weights, bw_method=self._bw
        )
...
class FlowHMM:
    def __init__(self, D, key=None):
        ...
        self._key = key
        self._D = D
        data = jax.random.beta(key, a=1.0, b=100.0, shape=(NUM_SAMPLES, D))
    ...
    def _fit_c(self, model_c, data, weights=None):
        ...
        def loss(model_c, args):
            ...
            l2 = jax.tree.reduce(
                operator.add, jax.tree.map(lambda x: jnp.sum(x**2), model_c)
            )
            ret = l1 + lam * l2
        ...
        with jax.debug_nans(True):
            soln = optx.minimise(
                loss, opt, model_c, args=(train, w_train, 1e-4), max_steps=None
            )
    ...
    def transition(
        self, fc: AbstractDistribution, s: jnp.ndarray, key
    ...
    def binom_sampling(
        self, fc, n: int, d: int, theta: jnp.ndarray, key
    ) -> AbstractDistribution:
    ...
    def forward(self, sln: Selection, prior_c, data: Dataset, key):
        ...
        (_, ll, _, _), (models, lls) = scan(self._forward_helper, init, seq)
    ...
    def _sample_path(self, models, sln: Selection, data: Dataset, key):
        ...
        model0, models = [
            jax.tree.map(lambda a: a[sl], models) for sl in [-1, slice(None, -1)]
        ]
    ...
    def em(self, sln0: Selection, data: Dataset, alpha=1.0, em_iterations=5):
        ...
        for i in range(em_iterations):
            ...
            gp.plot(
                ...
                _with="lines",
                title="lls",
                terminal="dumb 120,20",
                unset="grid",
            )
            gp.plot(
                *[(t[::-1], y[::-1]) for y in paths.T],
                _with="lines",
                title="afs",
